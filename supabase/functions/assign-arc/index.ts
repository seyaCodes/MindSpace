    ...
    [
      {
        role: "system",
        content:
          "Extract the core life topic from a therapeutic reflection. " +
          "Output exactly one sentence of at most 20 words. " +
          "Focus only on the factual life situation — no emotions, no therapeutic language, no questions.",
      },
      {
        role: "user",
        content: [
          "Extract the topic from this reflection:",
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // arc_topic is a short factual summary (≤20 words) generated by the LLM.
    // Embedding from this instead of raw reflection text means cosine similarity
    // clusters by life theme rather than by therapeutic/emotional writing style.
    let arcTopic: string;
    ...
    if (bestArc && bestSim >= SIMILARITY_THRESHOLD) {
      ...
      const { error: centErr } = await admin.from("arcs").update({ centroid: newCentroid }).eq("id", arcId);
      if (centErr) {
        console.error("[assign-arc][7] centroid update FAILED:", centErr.message, centErr.code);
      } else {
        console.log("[assign-arc][7] centroid updated OK");
      }
    } else {
      console.log(
